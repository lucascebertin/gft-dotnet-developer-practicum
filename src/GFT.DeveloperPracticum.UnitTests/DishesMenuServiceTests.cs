...
namespace GFT.DeveloperPracticum.UnitTests
{
    public class DishesMenuServiceTests
    {
        ...
        public void Should_allow_to_add_a_menu_dish_with_timeOfDay_and_dishType_when_empty()
        {
            ...
            var tuple = new Tuple<TimeOfDayType, DishType>(TimeOfDayType.Morning, DishType.Entree);
        ...
        [Fact]
        public void Should_not_allow_to_add_a_null_value_for_timeOfDay_and_dishType()
        {
            //arrange
            var dishesMenuService = new DishesMenuService();
            var dish = new Dish("eggs", AllowedOrderType.Single);
            ...
            //act
            var addDishResult = dishesMenuService.TryAddDish(null, dish);

            //assert
            addDishResult.Should().BeFalse("Because the key-pair will be rejected and will not be added to the dictionary");
        }

        [Fact]
        public void Should_not_allow_to_add_a_null_value_for_timeOfDay_and_dishType_and_for_dish()
        {
            //arrange
            var dishesMenuService = new DishesMenuService();

            //act
            var addDishResult = dishesMenuService.TryAddDish(null, null);

            //assert
            addDishResult.Should().BeFalse("Because the dish and the key-pair will be rejected and will not be added to the dictionary");
        }
